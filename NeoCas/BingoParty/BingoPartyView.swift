import SwiftUI

struct BingoPartyView: View {
    @StateObject var bingoPartyModel =  BingoPartyViewModel()
    @Environment(\.presentationMode) var presentationMode
    let columns: [GridItem] = Array(repeating: .init(.fixed(70), spacing: -7), count: 5)
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ZStack {
                ZStack(alignment: .top) {
                    ZStack {
                        Image(.partyBg)
                            .resizable()
                        
                        LinearGradient(
                            colors: [
                                Color(red: 89/255, green: 22/255, blue: 139/255),
                                Color(red: 163/255, green: 0/255, blue: 76/255),
                                Color(red: 15/255, green: 78/255, blue: 99/255)
                            ],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                        .opacity(0.6)
                    }
                    .ignoresSafeArea()
                    
                    Rectangle()
                        .fill(Color(red: 10/255, green: 3/255, blue: 31/255))
                        .overlay {
                            RoundedRectangle(cornerRadius: 0)
                                .stroke(Color(red: 12/255, green: 17/255, blue: 17/255), lineWidth: 3)
                        }
                        .frame(height: 180)
                        .ignoresSafeArea(edges: .top)
                }
                
                VStack {
                    VStack(spacing: 15) {
                        VStack(spacing: 20) {
                            HStack {
                                Button(action: {
                                    presentationMode.wrappedValue.dismiss()
                                }) {
                                    Rectangle()
                                        .fill(LinearGradient(colors: [Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                      Color(red: 0/255, green: 128/255, blue: 255/255),
                                                                      Color(red: 255/255, green: 1/255, blue: 255/255)], startPoint: .topLeading, endPoint: .bottomTrailing))
                                        .overlay {
                                            RoundedRectangle(cornerRadius: 8)
                                                .stroke(Color(red: 0/255, green: 255/255, blue: 255/255), lineWidth: 4)
                                                .overlay {
                                                    Image(systemName: "house")
                                                        .foregroundStyle(.black)
                                                }
                                        }
                                        .frame(width: 40, height: 32)
                                        .cornerRadius(8)
                                }
                                .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255), radius: 15)
                                .pressableButtonStyle()
                                
                                Spacer()
                                
                                Rectangle()
                                    .fill(LinearGradient(colors: [Color(red: 9/255, green: 44/255, blue: 68/255),
                                                                  Color(red: 20/255, green: 11/255, blue: 47/255)], startPoint: .topLeading, endPoint: .bottomTrailing))
                                    .overlay {
                                        RoundedRectangle(cornerRadius: 16)
                                            .stroke(Color(red: 9/255, green: 103/255, blue: 123/255), lineWidth: 2)
                                            .overlay {
                                                HStack(spacing: 5) {
                                                    Text("COINS:\(bingoPartyModel.coin)")
                                                        .FontMedium(size: 10)
                                                    
                                                    Image(.coin)
                                                        .resizable()
                                                        .frame(width: 24, height: 24)
                                                }
                                                .padding(.leading)
                                            }
                                    }
                                    .frame(width: 126, height: 32)
                                    .cornerRadius(16)
                                    .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.3), radius: 10, x: -5, y: -2)
                            }
                            
                            Text("NEON BINGO PARTY")
                                .FontHeavy(size: 30)
                                .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.3), radius: 15)
                                .overlay(
                                    LinearGradient(
                                        colors: [
                                            Color(red: 0/255, green: 255/255, blue: 255/255),
                                            Color(red: 0/255, green: 128/255, blue: 255/255),
                                            Color(red: 255/255, green: 1/255, blue: 255/255),
                                            Color(red: 0/255, green: 255/255, blue: 129/255)
                                        ],
                                        startPoint: .leading,
                                        endPoint: .trailing
                                    )
                                    .mask(
                                        Text("NEON BINGO PARTY")
                                            .FontHeavy(size: 30)
                                            .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.3), radius: 15)
                                    )
                                )
                        }
                        
                        ScrollView(showsIndicators: false) {
                            VStack(spacing: 15) {
                                Rectangle()
                                    .fill(LinearGradient(colors: [Color(red: 59/255, green: 37/255, blue: 107/255),
                                                                  Color(red: 84/255, green: 27/255, blue: 136/255),
                                                                  Color(red: 89/255, green: 41/255, blue: 153/255)], startPoint: .top, endPoint: .bottom).opacity(0.7))
                                    .frame(height: 84)
                                    .overlay {
                                        RoundedRectangle(cornerRadius: 20)
                                            .stroke(LinearGradient(colors: [Color(red: 40/255, green: 113/255, blue: 159/255),
                                                                            Color(red: 72/255, green: 88/255, blue: 170/255)], startPoint: .top, endPoint: .bottom), lineWidth: 3)
                                            .overlay {
                                                Text("N-43")
                                                    .FontHeavy(size: 60)
                                                    .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.3), radius: 15)
                                                    .overlay(
                                                        LinearGradient(
                                                            colors: [
                                                                Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                Color(red: 0/255, green: 128/255, blue: 255/255),
                                                                Color(red: 255/255, green: 1/255, blue: 255/255),
                                                                Color(red: 0/255, green: 255/255, blue: 129/255)
                                                            ],
                                                            startPoint: .leading,
                                                            endPoint: .trailing
                                                        )
                                                        .mask(
                                                            Text("N-43")
                                                                .FontHeavy(size: 60)
                                                        )
                                                    )
                                            }
                                    }
                                    .cornerRadius(20)
                                    .padding(.horizontal, 20)
                                
                                Rectangle()
                                    .fill(LinearGradient(
                                        colors: [
                                            Color(red: 0/255, green: 255/255, blue: 255/255),
                                            Color(red: 6/255, green: 95/255, blue: 184/255),
                                            Color(red: 168/255, green: 3/255, blue: 168/255),
                                            Color(red: 3/255, green: 167/255, blue: 83/255),
                                            Color(red: 84/255, green: 15/255, blue: 158/255)
                                        ],
                                        startPoint: .topLeading,
                                        endPoint: .bottomTrailing
                                    ))
                                    .frame(height: 688)
                                    .overlay {
                                        VStack(spacing: 10) {
                                            VStack(spacing: 15) {
                                                VStack {
                                                    BingoPlaceholder(text: "YOUR GUESS")
                                                    
                                                    LazyVGrid(columns: columns, spacing: 5) {
                                                        ForEach(bingoPartyModel.numbers.indices, id: \.self) { index in
                                                            let number = bingoPartyModel.numbers[index]
                                                            Button(action: {
                                                                if !bingoPartyModel.isAnimating {
                                                                    bingoPartyModel.userSelectedNumber = number
                                                                    bingoPartyModel.selectedNumber = number
                                                                }
                                                            }) {
                                                                Rectangle()
                                                                    .fill(bingoPartyModel.selectedNumber == number ?
                                                                          LinearGradient(colors: [Color.blue.opacity(0.7), Color.purple.opacity(0.7)],
                                                                                         startPoint: .topLeading, endPoint: .bottomTrailing) :
                                                                          LinearGradient(colors: [Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.2),
                                                                                                  Color(red: 6/255, green: 95/255, blue: 184/255).opacity(0.2)],
                                                                                         startPoint: .topLeading, endPoint: .bottomTrailing))
                                                                    .overlay(
                                                                        RoundedRectangle(cornerRadius: 14)
                                                                            .stroke(Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.5), lineWidth: 1)
                                                                            .overlay {
                                                                                if bingoPartyModel.showResultImage && bingoPartyModel.selectedNumber == number {
                                                                                    Image(.bingoParty)
                                                                                        .resizable()
                                                                                        .frame(width: 34, height: 34)
                                                                                } else {
                                                                                    Text("\(number)")
                                                                                        .FontBold(size: 18)
                                                                                        .foregroundColor(.white)
                                                                                }
                                                                            }
                                                                    )
                                                                    .frame(width: 51, height: 51)
                                                                    .cornerRadius(14)
                                                                    .padding(.horizontal, 5)
                                                            }
                                                            .disabled(bingoPartyModel.isAnimating)
                                                        }
                                                    }
                                                }
                                                
                                                VStack {
                                                    BingoPlaceholder(text: "YOUR BET")
                                                    Rectangle()
                                                        .fill(LinearGradient(
                                                            colors: [
                                                                Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                Color(red: 6/255, green: 95/255, blue: 184/255),
                                                                Color(red: 168/255, green: 3/255, blue: 168/255),
                                                                Color(red: 3/255, green: 167/255, blue: 83/255),
                                                                Color(red: 84/255, green: 15/255, blue: 158/255)
                                                            ],
                                                            startPoint: .topLeading,
                                                            endPoint: .bottomTrailing
                                                        ).opacity(0.2))
                                                        .overlay {
                                                            RoundedRectangle(cornerRadius: 8)
                                                                .stroke(Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.5), lineWidth: 1)
                                                                .overlay {
                                                                    HStack {
                                                                        Text("\(bingoPartyModel.bet)")
                                                                            .FontRegular(size: 16)
                                                                        Spacer()
                                                                    }
                                                                    .padding(.horizontal)
                                                                }
                                                        }
                                                        .frame(height: 36)
                                                        .cornerRadius(8)
                                                        .padding(.horizontal)
                                                    
                                                    HStack {
                                                        Button(action: {
                                                            bingoPartyModel.bet = bingoPartyModel.coin / 2
                                                        }) {
                                                            Rectangle()
                                                                .fill(LinearGradient(
                                                                    colors: [
                                                                        Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                        Color(red: 6/255, green: 95/255, blue: 184/255),
                                                                        Color(red: 168/255, green: 3/255, blue: 168/255),
                                                                        Color(red: 3/255, green: 167/255, blue: 83/255),
                                                                        Color(red: 84/255, green: 15/255, blue: 158/255)
                                                                    ],
                                                                    startPoint: .topLeading,
                                                                    endPoint: .bottomTrailing
                                                                ).opacity(0.2))
                                                                .overlay {
                                                                    RoundedRectangle(cornerRadius: 8)
                                                                        .stroke(Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.5), lineWidth: 1)
                                                                        .overlay {
                                                                            Text("1/2")
                                                                                .FontMedium(size: 14, color: Color(red: 0/255, green: 255/255, blue: 255/255))
                                                                        }
                                                                }
                                                                .frame(height: 36)
                                                                .cornerRadius(8)
                                                        }
                                                        
                                                        Button(action: {
                                                            if bingoPartyModel.bet * 2 <= bingoPartyModel.coin {
                                                                bingoPartyModel.bet *= 2
                                                            }
                                                        }) {
                                                            Rectangle()
                                                                .fill(LinearGradient(
                                                                    colors: [
                                                                        Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                        Color(red: 6/255, green: 95/255, blue: 184/255),
                                                                        Color(red: 168/255, green: 3/255, blue: 168/255),
                                                                        Color(red: 3/255, green: 167/255, blue: 83/255),
                                                                        Color(red: 84/255, green: 15/255, blue: 158/255)
                                                                    ],
                                                                    startPoint: .topLeading,
                                                                    endPoint: .bottomTrailing
                                                                ).opacity(0.2))
                                                                .overlay {
                                                                    RoundedRectangle(cornerRadius: 8)
                                                                        .stroke(Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.5), lineWidth: 1)
                                                                        .overlay {
                                                                            Text("2x")
                                                                                .FontMedium(size: 14, color: Color(red: 0/255, green: 255/255, blue: 255/255))
                                                                        }
                                                                }
                                                                .frame(height: 36)
                                                                .cornerRadius(8)
                                                        }
                                                    }
                                                    .padding(.horizontal)
                                                    
                                                    Button(action: {
                                                        if bingoPartyModel.userSelectedNumber == nil {
                                                              bingoPartyModel.showAlert = true
                                                          } else {
                                                              bingoPartyModel.showResultImage = false
                                                              bingoPartyModel.startAnimation()
                                                          }
                                                    }) {
                                                        Rectangle()
                                                            .fill(LinearGradient(colors: [Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                                          Color(red: 0/255, green: 128/255, blue: 255/255),
                                                                                          Color(red: 255/255, green: 1/255, blue: 255/255)], startPoint: .topLeading, endPoint: .bottomTrailing))
                                                            .overlay {
                                                                RoundedRectangle(cornerRadius: 8)
                                                                    .stroke(Color(red: 0/255, green: 255/255, blue: 255/255), lineWidth: 4)
                                                                    .overlay {
                                                                        HStack {
                                                                            Text("🎲  Generate Neural Signal  🎲")
                                                                                .FontBold(size: 14, color: .black)
                                                                        }
                                                                    }
                                                            }
                                                            .frame(height: 32)
                                                            .cornerRadius(8)
                                                    }
                                                    .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.3), radius: 15)
                                                    .padding(.horizontal)
                                                    .alert(isPresented: $bingoPartyModel.showAlert) {
                                                        Alert(title: Text("Selection Required"),
                                                              message: Text("Please select a number before starting."),
                                                              dismissButton: .default(Text("OK")))
                                                    }
                                                    .disabled(bingoPartyModel.isAnimating)
                                                }
                                                
                                                VStack {
                                                    BingoPlaceholder(text: "HISTORY")
                                                    
                                                    VStack {
                                                        ForEach(0..<3, id: \.self) { index in
                                                            let value = index < bingoPartyModel.guessHistory.count ? bingoPartyModel.guessHistory[index] : 0
                                                            Rectangle()
                                                                .fill(LinearGradient(
                                                                    colors: [
                                                                        Color(red: 0/255, green: 255/255, blue: 255/255),
                                                                        Color(red: 6/255, green: 95/255, blue: 184/255),
                                                                        Color(red: 168/255, green: 3/255, blue: 168/255),
                                                                        Color(red: 3/255, green: 167/255, blue: 83/255),
                                                                        Color(red: 84/255, green: 15/255, blue: 158/255)
                                                                    ],
                                                                    startPoint: .topLeading,
                                                                    endPoint: .bottomTrailing
                                                                ).opacity(0.2))
                                                                .overlay {
                                                                    RoundedRectangle(cornerRadius: 8)
                                                                        .stroke(Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.5), lineWidth: 1)
                                                                        .overlay {
                                                                            HStack {
                                                                                Text("Guess #\(index + 1)")
                                                                                    .FontRegular(size: 16)

                                                                                Spacer()

                                                                                Text("\(value)")
                                                                                    .FontRegular(size: 16)
                                                                            }
                                                                            .padding(.horizontal)
                                                                        }
                                                                }
                                                                .frame(height: 36)
                                                                .cornerRadius(8)
                                                                .padding(.horizontal)
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                        .shadow(color: Color(red: 0/255, green: 255/255, blue: 255/255).opacity(0.2), radius: 10)
                                    }
                                    .cornerRadius(20)
                                    .padding(.horizontal, 20)
                                
                            }
                            .padding(.top, UIScreen.main.bounds.width > 600 ? 60 : 15)
                        }
                    }
                }
                .padding([.top, .horizontal])
            }
        }
    }
}

#Preview {
    BingoPartyView()
}


